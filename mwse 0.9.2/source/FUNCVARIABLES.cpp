...
{
	VPVARIABLES pvars= 0;
	return machine.ReadMem((VPVOID)machine.reltolinear(LOCALVARIABLES_IMAGE),&pvars,sizeof(pvars))
		&& pvars
...
bool FUNCGETLOCAL::execute(void)
{
	...
	if(machine.GetRegister(SP,stackpointer))
	{
		...
		if(    machine.ReadMem((VPVOID)stackpointer,&stackparams,sizeof(stackparams))
			&& machine.ReadMem((VPVOID)machine.reltolinear(SCRIPTTARGETREF_IMAGE),&pref,sizeof(pref))
