...
namespace mwse
{
	...
	float xFirstStatic::execute(mwse::VMExecuteInterface &virtualMachine)
	{
		...
		if (dataHandler->currentInteriorCell != NULL) {
			reference = mwse::tes3::skipDeletedObjects<TES3::Reference>(dataHandler->currentInteriorCell->activators.head);
		}
		else {
			...
			if (cellPointer->size >= 1) {
				...
				reference = mwse::tes3::skipDeletedObjects<TES3::Reference>(cellPointer->cell->activators.head);
				int exteriorCount = 0;
				for (int i = 0; i < 9; i++) {
					...
					if (cellPointer->size >= 1) {
						TES3::Reference* tempReference = mwse::tes3::skipDeletedObjects<TES3::Reference>(cellPointer->cell->activators.head);
