...
`absolutePosAlignX <tes3uiElement/absolutePosAlignX.html>`_ (`number`_)
...
`absolutePosAlignY <tes3uiElement/absolutePosAlignY.html>`_ (`number`_)
    See absolutePosAlignY.

`alpha <tes3uiElement/alpha.html>`_ (`number`_)
...
`autoHeight <tes3uiElement/autoHeight.html>`_ (`boolean`_)
...
`autoWidth <tes3uiElement/autoWidth.html>`_ (`boolean`_)
    When ``true``, automatically expands element dimensions to fit child elements. Dimensions are restricted by minWidth, minHeight, maxWidth and maxHeight properties.

`borderAllSides <tes3uiElement/borderAllSides.html>`_ (`number`_)
...
`borderBottom <tes3uiElement/borderBottom.html>`_ (`number`_)
...
`childAlignY <tes3uiElement/childAlignY.html>`_ (`number`_)
    See childAlignX.

`childOffsetX <tes3uiElement/childOffsetX.html>`_ (`number`_)
    Integer number. View offset in pixels, applied to the position of child nodes. Used in scroll panes.

`childOffsetY <tes3uiElement/childOffsetY.html>`_ (`number`_)
    See childOffsetX.

`children <tes3uiElement/children.html>`_ (`table`_)
    A table containing references to child elements. This is a copy that does not track changes.

`color <tes3uiElement/color.html>`_ (`table`_)
...
`consumeMouseEvents <tes3uiElement/consumeMouseEvents.html>`_ (`boolean`_)
...
`id <tes3uiElement/id.html>`_ (`number`_)
...
`imageScaleX <tes3uiElement/imageScaleX.html>`_ (`number`_)
    Image scaling multipliers. Only applies to image elements.

`imageScaleY <tes3uiElement/imageScaleY.html>`_ (`number`_)
    Image scaling multipliers. Only applies to image elements.

`justifyText <tes3uiElement/justifyText.html>`_ (`string`_)
...
`maxHeight <tes3uiElement/maxHeight.html>`_ (`number`_)
    Maximum dimensions for auto-size layout and resizable frames. Integer number.

`maxWidth <tes3uiElement/maxWidth.html>`_ (`number`_)
    Maximum dimensions for auto-size layout and resizable frames. Integer number.

`minHeight <tes3uiElement/minHeight.html>`_ (`number`_)
    Minimum dimensions for auto-size layout and resizable frames. Integer number.

`minWidth <tes3uiElement/minWidth.html>`_ (`number`_)
    Minimum dimensions for auto-size layout and resizable frames. Integer number.

`name <tes3uiElement/name.html>`_ (`string`_)
    The element's name, taken from the name registered for the ID.

`paddingAllSides <tes3uiElement/paddingAllSides.html>`_ (`number`_)
...
`paddingBottom <tes3uiElement/paddingBottom.html>`_ (`number`_)
...
`paddingLeft <tes3uiElement/paddingLeft.html>`_ (`number`_)
...
`paddingRight <tes3uiElement/paddingRight.html>`_ (`number`_)
...
`paddingTop <tes3uiElement/paddingTop.html>`_ (`number`_)
...
`parent <tes3uiElement/parent.html>`_ (`tes3uiElement`_)
    A reference to the parent element.

`positionX <tes3uiElement/positionX.html>`_ (`number`_)
...
`positionY <tes3uiElement/positionY.html>`_ (`number`_)
...
`rawText <tes3uiElement/rawText.html>`_ (`string`_)
...
`width <tes3uiElement/width.html>`_ (`number`_)
    Element dimensions in pixels. Integer number.

`widthProportional <tes3uiElement/widthProportional.html>`_ (`boolean`_)
    Sets element dimensions using a proportional sizer. The sizer starts with the parent dimension in the flow direction, subtracts any fixed dimension children leaving the proportional sizer space. Each proportionally sized element then gets an equal division of the space, multiplied by this member. Values above 1.0 are permissible.
...
`wrapText <tes3uiElement/wrapText.html>`_ (`boolean`_)
...
`createBlock <tes3uiElement/createBlock.html>`_ (`method`_)
    Creates an empty block container inside the element. Used to group and layout elements.

`createButton <tes3uiElement/createButton.html>`_ (`method`_)
...
`createDivider <tes3uiElement/createDivider.html>`_ (`method`_)
...
`createFillBar <tes3uiElement/createFillBar.html>`_ (`method`_)
...
`createHorizontalScrollPane <tes3uiElement/createHorizontalScrollPane.html>`_ (`method`_)
...
`createHypertext <tes3uiElement/createHypertext.html>`_ (`method`_)
    Creates a text area with clickable words as links. Usage is still under research.

`createImage <tes3uiElement/createImage.html>`_ (`method`_)
...
`createImageButton <tes3uiElement/createImageButton.html>`_ (`method`_)
...
`createLabel <tes3uiElement/createLabel.html>`_ (`method`_)
...
`createNif <tes3uiElement/createNif.html>`_ (`method`_)
    Creates a NIF model from a file. Still under research.

`createParagraphInput <tes3uiElement/createParagraphInput.html>`_ (`method`_)
...
`createSliderVertical <tes3uiElement/createSliderVertical.html>`_ (`method`_)
...
`createTextInput <tes3uiElement/createTextInput.html>`_ (`method`_)
...
`createTextSelect <tes3uiElement/createTextSelect.html>`_ (`method`_)
...
`createThinBorder <tes3uiElement/createThinBorder.html>`_ (`method`_)
    Creates a styled thin border element. Any content should be created as children of this border.

`createVerticalScrollPane <tes3uiElement/createVerticalScrollPane.html>`_ (`method`_)
...
`destroy <tes3uiElement/destroy.html>`_ (`method`_)
...
`destroyChildren <tes3uiElement/destroyChildren.html>`_ (`method`_)
...
`findChild <tes3uiElement/findChild.html>`_ (`method`_)
...
`forwardEvent <tes3uiElement/forwardEvent.html>`_ (`method`_)
...
`getContentElement <tes3uiElement/getContentElement.html>`_ (`method`_)
...
`getPropertyBool <tes3uiElement/getPropertyBool.html>`_ (`method`_)
...
`getPropertyFloat <tes3uiElement/getPropertyFloat.html>`_ (`method`_)
...
`getPropertyInt <tes3uiElement/getPropertyInt.html>`_ (`method`_)
    Properties are named variables attached to an element. Gets a property value with ``propName`` as the property key. Morrowind uses these to bind variables to the UI. Useful for element class-specific properties. Returns 0 if the property key did not have data.

`getPropertyObject <tes3uiElement/getPropertyObject.html>`_ (`method`_)
...
`getTopLevelMenu <tes3uiElement/getTopLevelMenu.html>`_ (`method`_)
    Finds the parent menu containing the element. Useful for finding the correct menu to run layout updates on.

`loadMenuPosition <tes3uiElement/loadMenuPosition.html>`_ (`method`_)
...
`register <tes3uiElement/register.html>`_ (`method`_)
...
`registerAfter <tes3uiElement/registerAfter.html>`_ (`method`_)
...
`registerBefore <tes3uiElement/registerBefore.html>`_ (`method`_)
...
`reorderChildren <tes3uiElement/reorderChildren.html>`_ (`method`_)
...
`saveMenuPosition <tes3uiElement/saveMenuPosition.html>`_ (`method`_)
...
`setPropertyBool <tes3uiElement/setPropertyBool.html>`_ (`method`_)
...
`setPropertyFloat <tes3uiElement/setPropertyFloat.html>`_ (`method`_)
    Sets a property value with ``prop`` as the property key. Properties are named variables attached to an element. Gets a property value with ``propName`` as the property key. Morrowind uses these to bind variables to the UI. Useful for element class-specific properties.

`setPropertyInt <tes3uiElement/setPropertyInt.html>`_ (`method`_)
    Sets a property value with ``prop`` as the property key. Properties are named variables attached to an element. Gets a property value with ``propName`` as the property key. Morrowind uses these to bind variables to the UI. Useful for element class-specific properties.

`setPropertyObject <tes3uiElement/setPropertyObject.html>`_ (`method`_)
    Sets a property value with ``prop`` as the property key. Properties are named variables attached to an element. Gets a property value with ``propName`` as the property key. Morrowind uses these to bind variables to the UI. Useful for element class-specific properties.

`triggerEvent <tes3uiElement/triggerEvent.html>`_ (`method`_)
...
`unregister <tes3uiElement/unregister.html>`_ (`method`_)
    Unregisters an `event`_ handler.

`updateLayout <tes3uiElement/updateLayout.html>`_ (`method`_)
...
.. _``nil`` to reset them to the default value, which will deactivate any related layout mode.

Elements can have custom data attached using their `Property`: ../../lua/type/`nil`` to reset them to the default value, which will deactivate any related layout mode.

Elements can have custom data attached using their `Property.html
.. _``optional``
        Returning ``false`` may cancel an interaction for certain events. e.g. unfocus
