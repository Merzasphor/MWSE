return {
	...
	arguments = {{
		...
		tableParams = {
			...
			{ name = "playSound", type = "boolean", default = true, description = "If `false`, the up/down sound for the item won't be played. This only applies if `reference` is the player." },
			{ name = "showMessage", type = "boolean", default = false, description = "If `true`, a message box notifying the player will be shown. This only applies if `reference` is the player." },
			...
			{ name = "equipProjectiles", type = "boolean", default = true, description = "If `true`, and the reference has the same projectile already equipped, the stacks will be merged. This will only work if the GUI is updated." },
